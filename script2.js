document.addEventListener("DOMContentLoaded", function () {
   
    
let ball = {
    x: 10,
    y: 10,
    xInc: 3,
    yInc: 3,
    scale: 1,
    img: document.getElementById("basketball"),
    rotation: 0,

    // Draw the ball
    draw: function () {
        context.save();
        context.translate(this.x + this.img.width / 2, this.y + this.img.height / 2);
        context.rotate(this.rotation);
        context.scale(this.scale, this.scale);

        // Draw the ball
        context.drawImage(this.img, -this.img.width / 2, -this.img.height / 2);

        // Reset transformations
        context.restore();
    },

    // Move the ball
    move: function () {
        this.x += this.xInc;
        this.y += this.yInc;

        // Bounce off the left and right canvas edges
        if (this.x < 0 || this.x + this.img.width * this.scale > canvas.width) {
            this.xInc *= -1;
            this.scale += 0.1; // Scale up when hitting the edge

            // Ensure the ball stays within the canvas
            this.x = Math.max(0, Math.min(this.x, canvas.width - this.img.width * this.scale));
        }

        // Bounce off the top and bottom canvas edges
        if (this.y < 0 || this.y + this.img.height * this.scale > canvas.height) {
            this.yInc *= -1;

            // Ensure the ball stays within the canvas
            this.y = Math.max(0, Math.min(this.y, canvas.height - this.img.height * this.scale));
        }

        // Rotate the ball
        this.rotation += 0.01;
    },
};

let canvas = document.getElementById("myCanvas");
let context = canvas.getContext("2d");

// Draw ball at starting position
context.save();
ball.draw();
context.restore();

// Used to cancel animation
let animFrameId;

// Start the animation when the mouse is on the canvas
canvas.addEventListener("mouseover", function (e) {
    animFrameId = window.requestAnimationFrame(drawFrame);
});

// Stop the animation when the mouse is moved off the canvas
canvas.addEventListener("mouseout", function (e) {
    window.cancelAnimationFrame(animFrameId);
});

// Draw a single frame
function drawFrame() {
    context.clearRect(0, 0, canvas.width, canvas.height);
    ball.draw();
    ball.move();
    animFrameId = window.requestAnimationFrame(drawFrame);
}


});
